...
const Product = () => {
  ...
  useEffect(() => {
    ...
    const fetchImage = async () => {
      const response = await axios.get(
        `http://localhost:8081/api/v1/product/${id}/image`,
        { responseType: "blob" }
      );
  ...
  const deleteProduct = async () => {
    try {
      await axios.delete(`http://localhost:8081/api/v1/deleteproduct/${id}`);
      removeFromCart(id);
      ...
      <div className="containers" style={{ display: "flex" }}>
        ...
        <div className="right-column" style={{ width: "50%" }}>
          <div className="product-description">
            ...
            <p style={{ marginBottom: "1rem" }}>{product.productDesc}</p>
          </div>
